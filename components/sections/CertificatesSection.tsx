...
import { getCertificateKk } from "@/lib/i18n/content.kk";
import { t } from "@/lib/i18n/dictionary";
import type { Locale } from "@/lib/i18n/locale";
import MediaSlot from "@/components/ui/MediaSlot";
import RevealOnScroll from "@/components/ui/RevealOnScroll";

export default function CertificatesSection({ locale = "ru" }: { locale?: Locale }) {
  ...
  const items = CERTIFICATES.map((cert) => {
    const kk = locale === "kk" ? getCertificateKk(cert.filename) : undefined;
    return {
      ...cert,
      title: kk?.title ?? cert.title,
      description: kk?.description ?? cert.description,
      statusLabel: kk?.statusLabel ?? cert.statusLabel,
    };
  });
    ...
    <section className="bg-mist py-20 md:py-28">
      <div className="container-site">
        <RevealOnScroll className="max-w-xl">
          <p className="eyebrow">{dict.certificates.eyebrow}</p>
          <h2 className="mt-3 font-display text-3xl font-bold leading-[1.15] tracking-tight text-navy md:text-4xl">
            {dict.certificates.heading}
          </h2>
          <p className="mt-4 text-sm leading-relaxed text-slate">{dict.certificates.intro}</p>
        </RevealOnScroll>
          ...
          {items.map((cert, i) => (
            <RevealOnScroll key={cert.filename} delay={i * 50}>
              <button
                ...
                aria-label={`${dict.common.openDocument}: ${cert.title}`}
